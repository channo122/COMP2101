...
declare -a arr=($interface)
#This command is used to create a loop which will process each work in the array
for i in "${arr[@]}"
do
    if [[ $i = lo* ]] ; then continue ;
	fi
	# If Conditions for skipping loopback interface
	...
	#Lines are used for command’s output 
done
#Ending loop
#####
